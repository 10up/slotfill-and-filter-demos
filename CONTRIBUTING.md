...
The `develop` branch is the development branch which means it contains the next version to be released.  `master` contains the corresponding stable development version which is our current latest release.  Always work on the `develop` branch and open up PRs against `develop`.
